...
from tqdm.auto import tqdm
from constants import z_from_comoving_fast as _get_z_from_comoving_fast, dm_from_z, dm_from_z_fast
import os
...
from shared_utils import (equatorialFromGalactic, uniform_sphere_degrees, 
                         inject_uniform_healpix, apply_spectral_index, compare_flux_diff_to_error,
...
def _template_hash(lightcurves, sed_grid, names, t_grid, *, algo="blake2b", digest_size=16):
    payload = pickle.dumps(
        {"lcs": lightcurves, "sed": sed_grid, "names": names, "t_grid": t_grid},
        protocol=pickle.HIGHEST_PROTOCOL
    )
    h = hashlib.blake2b(payload, digest_size=digest_size) if algo=="blake2b" else hashlib.sha256(payload)
    return h.hexdigest()
...
def _atomic_joblib_dump(obj, path, compress=("zstd", 3)):
    try:
        ...
        atomic_save_pickle(obj, path)  # your existing safe fallback
    ...
    def from_catalog(cls,
        ...
        **This version requires a per-row Cenwave (Å) column**, ideally from `{name}_cenwave.csv`.
        ...
        for _, row in params.iterrows():
            ...
            med_vals = np.array([np.nanmedian(lamA[inv_b == i]) for i in range(uniq_bands.size)], dtype=float)
            med_lam_by_band = dict(zip(uniq_bands.tolist(), med_vals.tolist()))
            
            median_cenwave_by_band[name] = {str(b): float(L) for b, L in med_lam_by_band.items()}
        ...
        grid_meta = {
            "phase_grid": None, "z_grid": None,     # will be filled if a grid exists later
            "grid_version": 1,
            "phase_bin_step": float(PHASE_BIN_STEP),   # keep in sync with evaluator
        }
        payload = {
            ...
            "names": names,
            "sed_grid": sed_grid,
            "median_cenwave_by_band": median_cenwave_by_band,
            "meta": {
                "z": z_used,
                "dm": dm_list,
                "t0_cat": t0_cat_list,
        ...
        if save_to is not None:
            # prefer joblib+zstd (fast load) with fallback
            _atomic_joblib_dump(payload, save_to)
            self.template_file = str(save_to)
...
def get_median_cenwave_map(templates_or_path, event_name):
    ...
    def build_magnitude_grid(self, 
        ...
        # compute once for all z in the grid
        DMs = dm_from_z(z_grid).astype(np.float32)
        
        for i_tpl in range(n_templates):
            sed = self.sed_grid[i_tpl]
            for i_z, z in enumerate(z_grid):
                for filt in filters:
                    m_app = np.array([synthesize_mag_at_z(sed, ph, z, filt) for ph in phase_grid])
                    self.mag_grid[filt][i_tpl, i_z, :] = m_app - DMs[i_z]  # reuse precomputed DM
        
                
        # Store grid axes for interpolation
        self.mag_grid_axes = {'z': z_grid, 'phase': phase_grid}
            print(f"[build_magnitude_grid] Complete. Grid shape per filter: {self.mag_grid[filters[0]].shape}")
        
            # compile interpolators ONCE per process
    ...
    """
    Build per-filter, per-template RegularGridInterpolators from mag_grid.
    Runs once per process; NOT serialized to disk.
    """
    if not hasattr(self, "mag_grid") or not self.mag_grid:
        raise RuntimeError("mag_grid is not available. Build or load it first.")
    ...
    for filt, cube in self.mag_grid.items():
        # cube shape: (n_templates, n_z, n_phase)
        n_tpl = cube.shape[0]
        interps[filt] = [
    ...
    self._interps = interps  # persistent for this process
...
def diagnose_internal_consistency(
    ...
    *,
    ...
    if med_map:
        med_lam = pd.Series(med_map)  # index=band, values=Å
    else:
...
def synthesize_mag_at_z_cached(cache, sed_grid, phase_bin_idx, z, filt, *, step=PHASE_BIN_STEP):
    """
    Cache key: (id(sed_grid), filt, bin_idx, round(z,5)).
    We compute the *bin center* once here and hand it to the synthesizer.
    """
...
# ---- Phase bucketing (vectorized) ----
PHASE_BIN_STEP = 0.2  # days (rest-frame); keep consistent everywhere

def phase_bucket_vec(phase_rest, step=PHASE_BIN_STEP, *, return_index=True):
    """
    Vectorized phase quantization.
    If return_index=True: returns integer bin indices (stable keys).
    Otherwise: returns bin centers (float), i.e. idx * step.
    """
    pr = np.asarray(phase_rest, dtype=float)
    idx = np.rint(pr / float(step)).astype(np.int32)  # rint = bankers rounding, stable
    if return_index:
        return idx
...
def evaluate_slsn(self, dataSlice, slice_point, return_full_obs=True):
    """
    SLSN evaluator: rest-frame timing + mag-grid fast path, with
    diagnostic sampling, per-filter extinction shortcut, and safe K-corr hook.
    Returns (snr, filters, mjds, obs_record|None).
    """
    # --- Observations (subset before heavy work) ---
    mjd_all   = dataSlice[self.mjdCol].astype(float)
    m5_all    = dataSlice[self.m5Col].astype(float)
    filt_all  = dataSlice[self.filterCol].astype(str)

    t0_mjd = self.mjd0 + float(slice_point['peak_time'])
    z      = float(slice_point['z'])

    # Rest-frame phase (days)
    phase_all = (mjd_all - t0_mjd) / (1.0 + z)

    # Optional phase windowing (pre-trim for speed)
    win = getattr(self, "phase_window_rest", None)  # e.g., (5.0, 160.0)
    if win is not None:
        pre, post = float(win[0]), float(win[1])
        in_window = (phase_all >= -pre) & (phase_all <= post)
        if not np.any(in_window):
            if return_full_obs:
                return np.array([]), np.array([]), np.array([]), None
            return np.array([]), np.array([]), np.array([]), None
        mjd  = mjd_all[in_window]
        m5   = m5_all[in_window]
        filt = filt_all[in_window]
        phase = phase_all[in_window]
    else:
        mjd, m5, filt, phase = mjd_all, m5_all, filt_all, phase_all

    n = phase.size
    mags = np.full(n, np.nan, dtype=np.float32)

    # --- Choose engine: mag-grid preferred; template-interp fallback ---
    # in evaluate_slsn, grid path
    use_grid = hasattr(self.lc_model, "mag_grid") and bool(self.lc_model.mag_grid)
    tpl_idx  = int(np.clip(int(slice_point['file_indx']), 0, len(getattr(self.lc_model, "data", [])) - 1))
    dm       = float(slice_point.get('distance_modulus'))
    
    if use_grid:
        # Ensure interps were compiled once (cheap no-op if already present)
        if not hasattr(self.lc_model, "_interps"):
            self.lc_model._build_interpolators()
    
        for f in np.unique(filt):
            mask = (filt == f)
            interps_f = self.lc_model._interps.get(f)
            if interps_f is None:
                continue
            mags[mask] = interps_f[tpl_idx](np.column_stack([np.full(mask.sum(), z), phase[mask]]))
    
        # Grid holds ABSOLUTE magnitudes → add DM only
        mags += dm
    else:
    # --- SLOW PATH: template synthesis with per-event cache (vectorized bucketing) ---
        # Quantize once, build unique (filter, bin_idx) pairs as a structured array
        bin_idx = phase_bucket_vec(phase, step=PHASE_BIN_STEP, return_index=True)
    
        # Build a structured key to avoid dtype coercions
        dt = np.dtype([('f', 'U2'), ('b', np.int32)])
        keys = np.empty(bin_idx.size, dtype=dt)
        keys['f'] = filt.astype('U2')  # LSST filters are single char; 'U2' is safe
        keys['b'] = bin_idx
    
        uniq, inv = np.unique(keys, return_inverse=True)
    
        # Evaluate each unique pair once using the per-event cache
        mag_abs_uniq = np.full(uniq.size, np.nan, dtype=float)
        for k, (f_u, b_u) in enumerate(zip(uniq['f'], uniq['b'])):
            mag_abs_uniq[k] = synthesize_mag_at_z_cached(
                self._mag_cache, sed_grid=self.lc_model.sed_grid[tpl_idx],
                phase_bin_idx=b_u, z=z, filt=f_u, step=PHASE_BIN_STEP
            )
    
        # Scatter back and add DM (K-corr optional hook preserved)
        mags = mag_abs_uniq[inv] + dm
    
        if getattr(self, "use_kcorrect", False):
            # add K only where defined already (no new work)
            for f in np.unique(filt):
                mask = (filt == f) & np.isfinite(mags)
                if not np.any(mask): 
                    continue
                K_f = float(slice_point.get(f"K_{f}", 0.0))
                if K_f != 0.0:
                    mags[mask] += K_f

    # --- Extinction (prefer precomputed A_f in slice_point) ---
    ebv = float(slice_point.get('ebv', 0.0))
    if ebv != 0.0 or any(k.startswith("A_") for k in slice_point.keys()):
        ax = self.ax1  # dict of A_lambda / E(B-V) per LSST band
        for f in np.unique(filt):
            mask = (filt == f) & np.isfinite(mags)
            if not np.any(mask): continue
            A_f = slice_point.get(f"A_{f}")
            if A_f is None:
                A_f = ax.get(f, 0.0) * ebv
            mags[mask] = mags[mask] + float(A_f)

    # --- SNR (NaN-safe) ---
    snr = np.zeros_like(mags, dtype=np.float32)
    ok  = np.isfinite(mags) & np.isfinite(m5)
    if np.any(ok):
        snr[ok] = _m52snr(mags[ok], m5[ok])

    # --- Return paths ---
    if not return_full_obs:
        return snr, filt, mjd, None
    ...
    obs_record = {
        ...
        "mag_obs": mags.tolist(),
        "snr_obs": snr.tolist(),
        "phase_rest": phase.tolist(),
    }

    want_diag = getattr(self, "diag_store", False) and getattr(self, "store_obs_mode", "meta") in {"diag", "full"}
    if want_diag:
        if not hasattr(self, '_rng') or self._rng is None:
            self._rng = np.random.default_rng(12345)
        keep = np.ones_like(snr, dtype=bool)
        if getattr(self, 'diag_min_snr', None) is not None:
            keep &= (snr >= float(self.diag_min_snr))
        if getattr(self, 'diag_max_mag', None) is not None:
            keep &= (mags <= float(self.diag_max_mag))
        p = float(np.clip(getattr(self, 'diag_sample_rate', 0.0), 0.0, 1.0))
        if p > 0.0:
            keep &= (self._rng.random(size=keep.size) < p)
        idx = np.flatnonzero(keep)
        cap = int(getattr(self, 'diag_per_event_cap', 0) or 0)
        if cap > 0 and idx.size > cap:
            idx = self._rng.choice(idx, size=cap, replace=False)
        obs_record['diag_sample_mjd']    = mjd[idx].tolist() if idx.size else []
        obs_record['diag_sample_mag']    = mags[idx].tolist() if idx.size else []
        obs_record['diag_sample_snr']    = snr[idx].tolist() if idx.size else []
        obs_record['diag_sample_filter'] = filt[idx].tolist() if idx.size else []
    

    return snr, filt, mjd, obs_record
...
class SLSN_Base_Metric(BaseMetric):
    def __init__(self, *, metricName='BaseSLSNMetric', 
                 ...
                 k_correct_type=None, k_correct_arg=None, store_obs_mode=DEFAULT_STORE_MODE, diag_store=False,
                 **kwargs):
        ...
        self.diag_store = bool(diag_store)  # only matters if store_obs_mode in {"diag","full"}
...
class SLSN_Detect_Metric(SLSN_Base_Metric):
    ...
    def run(self, dataSlice, slice_point=None):
        ...
        # Always build a tiny meta dict (cheap and useful)
        meta = {
        ...
        if mode == "none":
            ...
            # Keep only the small diagnostic payload if present; do not store full arrays
            diag_keys = ("diag_sample_mjd","diag_sample_mag","diag_sample_snr","diag_sample_filter")
            diag = {k: obs_record.get(k, []) for k in diag_keys}
            rec = {**meta, **diag}
            self.obs_records[slice_point['sid']] = rec

        elif mode == "full":
            # Keep everything we already computed once; nothing extra is built here
            full = dict(obs_record)  # shallow copy
            full.update(meta)
            self.obs_records[slice_point['sid']] = full

        else:
            # Fallback: treat unknown modes as meta
            self.obs_records[slice_point['sid']] = meta

        return 1.0 if detected else 0.0

# In-memory cache for repeated sky draws within a process
...
def _cached_uniform_healpix(nside: int, n_events: int, seed: int,
    ...
    """
    Return RA, DEC for a uniform HEALPix draw. 
    Uses (nside, n_events, seed) as the key.
    If cache_file is provided, persist/reuse between runs.
    """
...
def generate_SLSN_PopSlicer(lc_model, t_start=1, t_end=3652,
                            ...
                            gal_lat_cut=None,
                            seed=42,
                            healpix_cache_file: Path | None = None,   # <— NEW
                            save_to=None,
                            load_from=None):
    """
    Generate population of SLSNe with redshift-dependent distances.
    Reuses cached sky draws (nside, n_events, seed) to avoid redraws.
    EBV and per-filter extinctions are persisted in the saved pickle.
    """
    # Fast reload path still wins
    ...
    # Volumetric rate → event count (unchanged)
    ...
    # Cached uniform sky (NEW)
    nside = 64
    ra, dec = _cached_uniform_healpix(nside, n_events, seed, cache_file=healpix_cache_file)

    # z from comoving volume + fast DM (constants.py)
    ...
    z_vals = z_from_comoving_fast(d_cm)  # same shared table as DM
    distances = d_cm
    ...
    # EBV (vectorized) — persisted in slice_points (unchanged, but highlighted)
    ...
    # Per-filter extinctions (persisted)
    ax1 = dust_model.ax1
    for f in ['u','g','r','i','z','y']:
        sp[f'A_{f}'] = ax1[f] * ebv_vals

    # Peak magnitude summaries (persisted)
    for f in ['u','g','r','i','z','y']:
        peak_abs, peak_noebv, peak_ebv = [], [], []
        for idx, z_val, dm, ebv in zip(file_indx, z_vals, sp['distance_modulus'], ebv_vals):
...
class SLSN_CharacterizeMetric(SLSN_Base_Metric):
    ...
    def run(self, dataSlice, slice_point=None):
        # IMPORTANT: request full obs so we can reuse mags/MJDs computed inside evaluate
        ...
        # 1) Must pass detection first (reuse arrays; no recomputation)
        ...
        # 2) Characterization criteria (reuse the same arrays)
        good = snr >= 5

        # (2) ≥5 epochs with SNR≥5
        n_epochs = int(np.sum(good))
        if n_epochs < 5:
            return 0.0

        # (3) ≥3 distinct filters among good detections
        n_filters = np.unique(filters[good]).size
        if n_filters < 3:
            return 0.0

        # (4) ≥2 epochs within ±10 days of peak (use MJDs from obs_record)
        ...
        # Store small metadata payload (already computed; no new work)
        obs_record.update({
            'characterized': True,
            'n_epochs': n_epochs,
...
class SLSN_SpecTriggerMetric(SLSN_Base_Metric):
    def run(self, dataSlice, slice_point=None):
        # Ask evaluator for the per-visit arrays we’ll reuse
        snr, filters, times, obs_record = evaluate(
            self, dataSlice, slice_point, return_full_obs=True
        )
        if obs_record is None or snr.size == 0:
        ...
        # Reuse the arrays already computed by evaluate (no new work)
        mags    = np.asarray(obs_record['mag_obs'], float)
        mjd_obs = np.asarray(obs_record['mjd_obs'], float)

        # Use the same detection gate as the Detect metric
        detected = detect_slsn(filters, snr, mjd_obs, mags, obs_record)
        if not detected:
            return 0.0

        # Near-peak requirement: at least one SNR≥5 epoch within ±5 days of peak
        peak_mjd  = self.mjd0 + float(slice_point['peak_time'])
        near_peak = (snr >= 5) & (np.abs(mjd_obs - peak_mjd) <= 5.0)
        if not np.any(near_peak):
            return 0.0

        # Brightness requirement near peak
        if np.min(mags[near_peak]) > 21.0:
            return 0.0

        # Optional color check near peak (only if both bands exist)
        ...
        # Store a small, already-computed payload (no recomputation)
        obs_record.update({
            'spec_trigger': True,
